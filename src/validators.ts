...
function isPrivateIpv4(ip: string): boolean {
  ...
  if (ip.startsWith("172.")) {
    ...
    if (
      ...
      secondOctet >= PRIVATE_172_RANGE.min &&
      secondOctet <= PRIVATE_172_RANGE.max
    ) {
      return true;
    }
  }
 ...
 */
export function validateToolEndpoint(endpoint: string): void {
  let parsed: URL;
  try {
  ...
  if (parsed.protocol !== "https:") {
    throw new Error(
      `Tool endpoint must use HTTPS (got ${parsed.protocol.replace(":", "")}): ${endpoint}`,
    );
  }
  ...
  if (looksLikeIpv4(bare) && isPrivateIpv4(bare)) {
    throw new Error(
      `Tool endpoint must not target a private IP address: ${endpoint}`,
    );
  }

  if (bare.includes(":") && isPrivateIpv6(bare)) {
    throw new Error(
      `Tool endpoint must not target a private IPv6 address: ${endpoint}`,
    );
  }
}
 ...
 */
export function validateToolSsmPath(
  businessId: string,
  ssmPath: string,
): void {
  ...
  if (!ssmPath.startsWith(requiredPrefix)) {
    throw new Error(
      `auth_ssm_path must start with "${requiredPrefix}" (got "${ssmPath}")`,
    );
  }

  if (ssmPath.includes("..")) {
    throw new Error(
      `auth_ssm_path must not contain path traversal sequences: "${ssmPath}"`,
    );
  }
}

// ── Combined tool validation ────────────────────────────────────────── //
  ...
  for (const tool of tools) {
    if (tool.endpoint) {
      try {
        ...
        throw new Error(
          `Tool "${tool.name}": ${err instanceof Error ? err.message : String(err)}`,
        );
      }
    }
